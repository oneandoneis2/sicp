...
; 4.44
(define (eight-queens)
  (define (column num) (list num (amb 1 2 3 4 5 6 7 8)))
  (define (none? lst)
    (if (null? lst)
      #t
      (if (eq? #f (car lst))
        (none? (cdr lst))
        #f)))
  (define (get_remainder colnum board)
    (if (eq? colnum (caar board))
      (cdr board)
      (get_remainder colnum (cdr board))))
  (define (hasdiag? board hi lo)
    (if (null? board)
      #f
      (if (or (eq? (caar board) lo)
              (eq? (caar board) hi))
        (amb)
        (hasdiag? (cdr board) (+ hi 1) (- lo 1))))
    )
  (define (nodiags? board)
    (none? (map (lambda (column)
                  (let ((colnum (car column))
                        (colval (cadr column)))
                    (let ((remboard (get_remainder colnum board)))
                      (hasdiag? remboard (+ colval 1) (- colval 1)))))
                board)))
  (let ((board (list (column 1)(column 2)
                     (column 3)(column 4)
                     (column 5)(column 6)
                     (column 7)(column 8))))
    (require (distinct? board)) ; Same row = an attack
    (require (nodiags? board))  ; On a diagonal = an attack
    board))
